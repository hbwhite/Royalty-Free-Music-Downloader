...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        ...
        if (songCount > 0) {
            Album *album = [delegate singleAlbumViewControllerAlbum];
...
- (NSFetchedResultsController *)fetchedResultsController {
    if ((mode == kVisibilityViewControllerModeEdit) || (mode == kVisibilityViewControllerModeMultiEdit)) {
        if (!editingFetchedResultsController) {
            ...
            if ([[NSUserDefaults standardUserDefaults]boolForKey:kGroupByAlbumArtistKey]) {
                [fetchRequest setPredicate:[NSPredicate predicateWithFormat:@"(albumRefForAlbumArtistGroup == %@) AND (iPodMusicLibraryFile == %@)", [delegate singleAlbumViewControllerAlbum], [NSNumber numberWithBool:NO]]];
            }
            else {
                [fetchRequest setPredicate:[NSPredicate predicateWithFormat:@"(albumRefForArtistGroup == %@) AND (iPodMusicLibraryFile == %@)", [delegate singleAlbumViewControllerAlbum], [NSNumber numberWithBool:NO]]];
            }
        ...
        if (!fetchedResultsController) {
            ...
            if ([[NSUserDefaults standardUserDefaults]boolForKey:kGroupByAlbumArtistKey]) {
                [fetchRequest setPredicate:[NSPredicate predicateWithFormat:@"albumRefForAlbumArtistGroup == %@", [delegate singleAlbumViewControllerAlbum]]];
            }
            else {
                [fetchRequest setPredicate:[NSPredicate predicateWithFormat:@"albumRefForArtistGroup == %@", [delegate singleAlbumViewControllerAlbum]]];
            }
