...
- (void)groupByAlbumArtistPreferenceDidChange {
    ...
    dispatch_async(mainQueue, ^{
        // [NSFetchedResultsController deleteCacheWithName:@"Local_Song_Cache"];
        // [NSFetchedResultsController deleteCacheWithName:@"Universal_Song_Cache"];
...
- (NSFetchedResultsController *)fetchedResultsController {
    if ((mode == kVisibilityViewControllerModeEdit) || (mode == kVisibilityViewControllerModeMultiEdit)) {
        if (!editingFetchedResultsController) {
            ...
            Artist *artist = nil;
            if (delegate) {
                if ([delegate respondsToSelector:@selector(songsViewControllerArtist)]) {
                    artist = [delegate songsViewControllerArtist];
                }
            }
            if (artist) {
                if ([[NSUserDefaults standardUserDefaults]boolForKey:kGroupByAlbumArtistKey]) {
                    [fetchRequest setPredicate:[NSPredicate predicateWithFormat:@"(artistRefForAlbumArtistGroup == %@) AND (iPodMusicLibraryFile == %@)", artist, [NSNumber numberWithBool:NO]]];
                }
                else {
                    [fetchRequest setPredicate:[NSPredicate predicateWithFormat:@"(artistRefForArtistGroup == %@) AND (iPodMusicLibraryFile == %@)", artist, [NSNumber numberWithBool:NO]]];
                }
            }
            else {
            ...
            NSFetchedResultsController *aFetchedResultsController = [[NSFetchedResultsController alloc]initWithFetchRequest:fetchRequest managedObjectContext:managedObjectContext sectionNameKeyPath:@"fileSectionTitle" cacheName:/* (!artist) ? @"Local_Song_Cache" : */ nil];
            aFetchedResultsController.delegate = self;
            editingFetchedResultsController = aFetchedResultsController;
            
            // @try {
                if (![editingFetchedResultsController performFetch:nil]) {
                    aFetchedResultsController = [[NSFetchedResultsController alloc]initWithFetchRequest:fetchRequest managedObjectContext:managedObjectContext sectionNameKeyPath:nil cacheName:/* (!artist) ? @"Local_Song_Cache" : */ nil];
                    aFetchedResultsController.delegate = self;
                    editingFetchedResultsController = aFetchedResultsController;
        ...
        if (!fetchedResultsController) {
            ...
            if (artist) {
                if ([[NSUserDefaults standardUserDefaults]boolForKey:kGroupByAlbumArtistKey]) {
                    [fetchRequest setPredicate:[NSPredicate predicateWithFormat:@"artistRefForAlbumArtistGroup == %@", artist]];
                }
                else {
                    [fetchRequest setPredicate:[NSPredicate predicateWithFormat:@"artistRefForArtistGroup == %@", artist]];
                }
            }
            
            NSFetchedResultsController *aFetchedResultsController = [[NSFetchedResultsController alloc]initWithFetchRequest:fetchRequest managedObjectContext:managedObjectContext sectionNameKeyPath:@"fileSectionTitle" cacheName:/* (!artist) ? @"Universal_Song_Cache" : */ nil];
            aFetchedResultsController.delegate = self;
            fetchedResultsController = aFetchedResultsController;
            
            // @try {
                if (![fetchedResultsController performFetch:nil]) {
                    aFetchedResultsController = [[NSFetchedResultsController alloc]initWithFetchRequest:fetchRequest managedObjectContext:managedObjectContext sectionNameKeyPath:nil cacheName:/* (!artist) ? @"Universal_Song_Cache" : */ nil];
                    aFetchedResultsController.delegate = self;
                    fetchedResultsController = aFetchedResultsController;
